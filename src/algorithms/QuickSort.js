...
export default class QuickSort extends SortAlgorithm {
  ...
  if (low < high) {
    ...
    generateTrace(array) {
        ...
        for(let i=0; i<n; i++) if(!sortedIndices.has(i)) sortedIndices.add(i);
        trace.push(this.createStep('finish', [], arr, Array.from(sortedIndices), 'Sorting Complete', {}, [], callStack));
        return trace;
    }

    quickSortHelper(arr, low, high, trace, sortedIndices, callStack) {
        if (low < high) {
            trace.push(this.createStep('highlight', [], arr, Array.from(sortedIndices), `Condition low < high met`, { low, high }, [2], callStack));
            ...
            // pi is now sorted
            if (!sortedIndices.has(pi)) sortedIndices.add(pi);
            trace.push(this.createStep('sorted', [pi], arr, Array.from(sortedIndices), `Pivot ${arr[pi]} is in correct position`, { low, high, pi }, [3], callStack));

            callStack.push(`quickSort(arr, ${low}, ${pi - 1})`);
            trace.push(this.createStep('highlight', [], arr, Array.from(sortedIndices), `Sorting left of pivot`, { low, high, pi }, [4], callStack));
            this.quickSortHelper(arr, low, pi - 1, trace, sortedIndices, callStack);
            callStack.pop();

            callStack.push(`quickSort(arr, ${pi + 1}, ${high})`);
            trace.push(this.createStep('highlight', [], arr, Array.from(sortedIndices), `Sorting right of pivot`, { low, high, pi }, [5], callStack));
            this.quickSortHelper(arr, pi + 1, high, trace, sortedIndices, callStack);
            callStack.pop();
        } else if (low === high) {
             if (!sortedIndices.has(low)) sortedIndices.add(low);
        }
    }
